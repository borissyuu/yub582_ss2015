...
void main() {
	...
	time *= vec2(-1., 0.0)*random(grid); //direction
	//time *= vec2(-1., 0.0) * random(i_st.y);

	vec2 offset = vec2(0.1, 0.);
	vec2 tile = truchetPattern(i_st, random(f_st));
	vec3 color = vec3(1.0);
	//color = smoothstep(tile.x-.3, tile.x, tile.y) - smoothstep(tile.x, tile.x+.3, tile.y);
	color *= step(grid.y, i_st.y);
	color += step(tile.x, i_st.y*time.y);

	color.r *= random(floor((st*st)*time*20.+offset));
